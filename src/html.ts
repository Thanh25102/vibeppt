...
function nativeText(
  tag: "h1" | "h2" | "h3" | "p" | "span" | "strong" | "small",
  id: string,
  text: unknown,
  className: string,
...
function renderChrome(flat: FlatSlide, total: number, brand: BrandProfile, theme: ThemeName, assetMap: Map<string, string>): string {
  ...
  return `<header class="vp-chrome">
    ...
    <div class="vp-meta"><span>${escapeHtml(flat.sectionTitle)}</span><b>${String(flat.slideIndex + 1).padStart(2, "0")} / ${String(total).padStart(2, "0")}</b></div>
  </header>`;
}
...
function renderTitle(slide: SlideSpec, hero = false): string {
  return nativeText(hero ? "h1" : "h2", `${slide.id}-title`, slide.title, hero ? "vp-title vp-title-hero" : "vp-title");
}
...
function renderVisual(slide: SlideSpec, assetMap: Map<string, string>, className = "vp-visual"): string {
  ...
  return `<figure class="${className}"><img src="${escapeHtml(src)}" alt="${escapeHtml(slide.visual.alt)}" style="object-fit:${slide.visual.fit ?? "cover"};object-position:${escapeHtml(slide.visual.position ?? "center")}">${slide.visual.caption ? `<figcaption>${escapeHtml(slide.visual.caption)}</figcaption>` : ""}</figure>`;
}
...
function diagramFromItems(slide: SlideSpec): DiagramSpec {
  ...
  return {
    layout: slide.kind === "timeline" ? "horizontal" : "horizontal",
    nodes,
...
(() => {
  ...
  function measure(index) {
    ...
    const texts = [...slide.querySelectorAll('[data-ppt-native="text"]')].map((element) => {
      ...
      return {
        kind:'text', id:element.dataset.pptId, text:element.innerText,
        ...box(element, root), fontFamily:style.fontFamily.split(',')[0].replaceAll('"','').trim(),
        fontSizePx:parseFloat(style.fontSize), fontWeight:parseInt(style.fontWeight,10)||400,
        color:rgbToHex(style.color), align:['center','right'].includes(style.textAlign)?style.textAlign:'left',
